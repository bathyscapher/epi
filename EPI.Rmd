...
Luckily, an [ISO standard exists](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3) that allocates a unique 3-letter code to every country.
...
Some of these islands actually belong to a nation; e.g., Greenland to Denkmark and Svalbard to Norway (for now, keep it as is although it would be reasonable to merge these islands with their mainlands).
...
# Choropleth map
Now, let's render an interactive map with [R leaflet](https://cran.r-project.org/web/packages/leaflet/index.html) to display the EPI on the globe.
...
`r format(round(cor(epi.long[epi.long$Type == "EPI", ]$EPI.new.value, 
      epi.long[epi.long$Type == "EPI", ]$X2019, use = "complete.obs"),
      digits = 2), nsmall = 2)`, but increases to
`r round(cor(epi.long[epi.long$Type == "EPI", ]$EPI.new.value, 
      log(epi.long[epi.long$Type == "EPI", ]$X2019), use = "complete.obs"), 
...
Now, let's use a toy [structural equation model](https://en.wikipedia.org/wiki/Structural_equation_modeling) to reveal the driving forces behind the EPI.
...
## Specify the model
The assumption is that the EPI is positively driven by the GDP, the population size and the latitude with following reasoning:

* the GDP as a measure of economic development is in a sense a prerequisite for environmental development
...
These assumptions are visualized below in a [directed acyclic graph](https://en.wikipedia.org/wiki/Directed_acyclic_graph).

![](static/DAG.svg){width=45%}
...
This path diagram shows, that based on the available data, the strongest direct influence on the EPI is the latitude followed by the GDP and in a negative way, the population size. 
...
Another variable that could be an interesting extension to the dataset is the [Democracy Index](https://www.eiu.com/n/campaigns/democracy-index-2020/) as it seems plausible that developped democracies have the tendency to invest more into environmental development.
